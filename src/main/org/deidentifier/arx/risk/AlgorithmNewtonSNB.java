...
class AlgorithmNewtonSNB extends AlgorithmNewtonRaphson {
    ...
    protected double[][] firstDerivativeMatrix(final double[] iteratedSolution) {
        ...
        // Formula 2d, d alpha
        result[1][0] = (val9 * Math.log(val2) * val19 * val16) / (val10) - (val9 * Math.pow(val16, 2d) * 
                       val14) / (val10) - (val7 * val20 * val16) / (val10) - (a * val7 * val18 * Math.log(b) * val19 * val16) / (val10);
        // Formula 2d, d beta
        result[1][1] = -(val9 * val19) / (val10) - (Math.pow(a, 2d) * Math.pow(b, val15) * val20 * val16) / val10 - (a * val7 * 
