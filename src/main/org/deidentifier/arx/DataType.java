...
public abstract class DataType<T> implements Serializable, Comparator<T> { // NO_UCD
	...
	public static class ARXDate extends DataType<Date> implements DataTypeWithFormat, DataTypeWithRatioScale<Date> {
		...
		private static final DataTypeDescription<Date> description = new DataTypeDescription<Date>(Date.class,
				"Date/Time", DataScale.INTERVAL, true, listDateFormats()) {
			...
			@Override
			public DataType<Date> newInstance(String format) {
				return createDate(format);
			}

			@Override
			public DataType<Date> newInstance(String format, Locale locale) {
				return createDate(format, locale);
			}
		};
		...
		/**
		 * Create a date with a "dd.MM.yyyy" format string for
		 * <code>SimpleDateFormat</code> and default locale.
		 * 
		 * @see <a href=
		...
		/**
		 * Create a date with a format string and the default locale. Format
		 * strings must be valid formats for <code>SimpleDateFormat</code>.
		 *
		 * @param formatString
		 * @see <a href=
		 *      "http://docs.oracle.com/javase/7/docs/api/java/text/SimpleDateFormat.html">SimpleDateFormat</a>
		...
		/**
		 * Create a date with a format string and a given locale. Format strings
		 * must be valid formats for <code>SimpleDateFormat</code>.
		 *
		 * @param formatString
		 * @param locale
		 * @see <a href=
		...
		public Date multiply(Date multiplicand, double multiplicator) {
			long d1 = multiplicand.getTime();
			return new Date((long) (d1 * multiplicator));
		}
	...
	public static class ARXDecimal extends DataType<Double>
			implements DataTypeWithFormat, DataTypeWithRatioScale<Double> {

		/** SVUID */
		private static final long serialVersionUID = 7293446977526103610L;

		/** The description of the data type. */
		private static final DataTypeDescription<Double> description = new DataTypeDescription<Double>(Double.class,
				"Decimal", DataScale.RATIO, true, listDecimalFormats()) {
			...
			@Override
			public DataType<Double> newInstance(String format) {
				return createDecimal(format);
			}

			@Override
			public DataType<Double> newInstance(String format, Locale locale) {
				return createDecimal(format, locale);
			}
		};
		...
		/**
		 * Create a numeric with a format string and default locale. Format
		 * strings must be valid formats for <code>DecimalFormat</code>.
		 * 
		 * @param format
		 * @see <a href=
		 *      "http://docs.oracle.com/javase/7/docs/api/java/text/DecimalFormat.html">DecimalFormat</a>
		 */
		...
		/**
		 * Create a numeric with a format string and given locale. Format
		 * strings must be valid formats for <code>DecimalFormat</code>.
		 * 
		 * @param format
		 * @param locale
	...
	public static class ARXInteger extends DataType<Long> implements DataTypeWithFormat, DataTypeWithRatioScale<Long> {
		...
		/** The description of the data type. */
		private static final DataTypeDescription<Long> description = new DataTypeDescription<Long>(Long.class,
				"Integer", DataScale.RATIO, false, new ArrayList<String>()) {
			...
			@Override
			public DataType<Long> newInstance(String format) {
				return createInteger(format);
			}

			@Override
			public DataType<Long> newInstance(String format, Locale locale) {
				return createInteger(format, locale);
			}
		};
		...
		/**
		 * Create a numeric with a format string. Format strings must be valid
		 * formats for <code>DecimalFormat</code>.
		 *
		 * @param format
		 * @param locale
		 * @see <a href=
	...
	public static class ARXOrderedString extends DataType<String> implements DataTypeWithFormat {
		...
		/** The description of the data type. */
		private static final DataTypeDescription<String> description = new DataTypeDescription<String>(String.class,
				"Ordinal", DataScale.ORDINAL, true, new ArrayList<String>()) {
			...
			@Override
			public DataType<String> newInstance(String format) {
				return createOrderedString(format);
			}

			@Override
			public DataType<String> newInstance(String format, Locale locale) {
				return createOrderedString(format);
			}
		};
		 ...
		 *
		 * @param format
		 *            Ordered list of string separated by line feeds
		 */
	...
	public static class ARXString extends DataType<String> {
		...
		/** The description of the data type. */
		private static final DataTypeDescription<String> description = new DataTypeDescription<String>(String.class,
				"String", DataScale.NOMINAL, false, new ArrayList<String>()) {
	...
	public static abstract class DataTypeDescription<T> implements Serializable {
		 ...
		 */
		private DataTypeDescription(Class<T> clazz, String label, DataScale scale, boolean hasFormat,
		...
		/**
		 * Returns whether the type be parameterized with a format string. Note
		 * that every data type can be instantiated without a format string,
		...
		/**
		 * Creates a new instance with the given format string and default
		 * locale.
		 *
		 * @param format
		 * @return
		...
		/**
		 * Creates a new instance with the given format string and the given
		 * locale.
		 *
		 * @param format
		 * @param locale
	...
	public static interface DataTypeWithRatioScale<T> {
		 ...
		 * @return
		 */
		public abstract String multiply(String multiplicand, String multiplicator);
	 ...
	 * 
	 * @param format
	 *            List of ordered strings separated by line feeds
	 * @return
	 */
	...
	/**
	 * Provides a list of example formats for the <code>Decimal</code> data
	 * type.
	 *
	 * @return
	 */
	private static List<String> listDecimalFormats() {
		...
		for (Locale locale : NumberFormat.getAvailableLocales()) {
			for (NumberFormat format : new NumberFormat[] { NumberFormat.getNumberInstance(locale),
					NumberFormat.getIntegerInstance(locale), NumberFormat.getCurrencyInstance(locale),
	...
	/**
	 * Compares two values. The result is 0 if both values are equal, less than
	 * 0 if the first value is less than the second argument, and greater than 0
	 * if the first value is greater than the second argument.
	 * 
	 * @param s1
	 * @param s2
	 * @return
