    ...
    protected ImportAdapterJDBC(ImportConfigurationJDBC config) throws IOException {
        ...
        try {
            ...
            if (config.getMaxRows() != 0) {
                ...
                switch (databaseType) {
                    case "mysql":
                        querySql += " limit " + config.getMaxRows();
                        break;
                    case "oracle":
                        querySql += " WHERE rownum <=" + config.getMaxRows();
                        break;
                    case "db2":
                        querySql += " fetch first " + config.getMaxRows() + " rows only";
                        break;
                    case "sqlserver":
                        querySql = "select top " + config.getMaxRows() + " * " + "from " + afterTableName;
                        break;
                    default:
                        querySql += " limit " + config.getMaxRows();
                        break;
    ...
    public String[] next() {
        ...
        try {
            ...
            for (int i = 0; i < indexes.length; i++) {
                int columnType = resultSet.getMetaData().getColumnType(indexes[i]);
                ...
                switch (columnType) {
                    ...
                    case Types.TIMESTAMP:
                        result[i] = IOUtil.trim(String.valueOf(resultSet.getTimestamp(indexes[i])));
                    default:
            ...
            throw new RuntimeException("Couldn't retrieve data from database: " + msg);
        }
    }
