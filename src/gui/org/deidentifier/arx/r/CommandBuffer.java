...
public class CommandBuffer {
	...
	public void appendToCommandBuffer(String command) {
		...
		if (length < commandBuffer.length)
			length++;
	}
	...
	public String[] getLastCommands() {
		...
		int templ = length;
		// from offset backwards to 0, from length-1 backwards to either empty
		// string(/null?) or offset+1.
		int j = 0;
		...
		if (templ > 0) {
			// If templ > 0 length should be equal commandBuffer.length and there is no null
			// case
