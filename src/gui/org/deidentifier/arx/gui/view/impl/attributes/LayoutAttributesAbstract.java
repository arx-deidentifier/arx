 ...
 */
public class LayoutAttributesAbstract implements ILayout, IView {

    /** Model */
    private final Map<Integer, ViewAttributes<?>> views   = new HashMap<Integer, ViewAttributes<?>>();
    /** Model */
     ...
     * @param top
     */
    public LayoutAttributesAbstract(Composite parent, final Controller controller, boolean input, boolean top) {
     ...
     */
    public ViewAttributesType getSelectedViewType() {
     ...
     * @return
     */
    public ViewAttributes<?> getViewForSelectionIndex(final int index) {
    ...
    private void pullEnableState() {
        ...
        ViewAttributes<?> view = getViewForSelectionIndex(this.getSelectionIndex());
        if (view == null) {
            buttonEnable.setEnabled(false);
            ...
            boolean enabled;
            if (this.isInput) {
                enabled = model.getAttributesModel().isViewEnabledForInput(view.getViewType());
                buttonEnable.setSelection(enabled);
                if (view.isEnabled() != enabled) {
                    view.setEnabled(enabled);
                } 
            } else {
                enabled = model.getAttributesModel().isViewEnabledForOutput(view.getViewType());
                buttonEnable.setSelection(enabled);
                if (view.isEnabled() != enabled) {
                    view.setEnabled(enabled);
                }
    ...
    private void pushEnableState() {
        ...
        if (view != null) {
            ...
            if (this.isInput) {
                if (enabled != model.getAttributesModel().isViewEnabledForInput(view.getViewType())) {
                    model.getAttributesModel().setViewEnabledForInput(view.getViewType(), enabled);
                    view.setEnabled(enabled);
                } 
            } else {
                if (enabled != model.getAttributesModel().isViewEnabledForOutput(view.getViewType())) {
                    model.getAttributesModel().setViewEnabledForOutput(view.getViewType(), enabled);
                    view.setEnabled(enabled);
                }
            }
        }
     ...
     * @param view
     */
    protected void registerView(int index, ViewAttributes<?> view) {
