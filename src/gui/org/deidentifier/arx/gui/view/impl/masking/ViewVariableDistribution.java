...
public class ViewVariableDistribution implements IView {
	...
	private void build(Composite parent) {
		...
		ComponentTitledFolder folder = new ComponentTitledFolder(parent, controller, null, null); // TODO
																									// Assign
																									// help
																									// id
	...
	/**
	 * Returns a double array containing Y values of this distribution for this
	 * X values.
	 * 
	 * @param xSeries
	 * @param result
	...
	public void update(ModelEvent event) {
		...
		if (event.part == ModelPart.RANDOM_VARIABLE) {
			...
			if (event.subpart == 1) {
				...
				chart.getAxisSet().getYAxis(0).getTitle().setText(Resources.getMessage("VariableDistributionView.0"));
				chart.getAxisSet().getXAxis(0).getTitle().setText(Resources.getMessage("VariableDistributionView.1"));
				chart.redraw();
			} else {
